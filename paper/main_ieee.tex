...
\textbf{Robustness and additional validation.} \simp{We use the Dow 30 (fixed as of 2018) to avoid survivorship bias, but our results may not apply to other stock universes or market segments. To test robustness, we suggest: (i) rolling evaluations with expanding training windows; (ii) different split points (like using 2019 for validation instead of 2020); and (iii) testing on other stock universes like the S\&P 100. We use time-ordered splits rather than random splits because random splits can leak future information and break the time dependencies that are crucial for financial data. We focus on one main split for clarity, but these robustness tests can be easily done with our code.}
    	...
    	\rev{The coefficient of determination $\rTwo$ is computed against a zero-return baseline as:}
    	\begin{equation}
    	\rev{\rTwo = 1 - \frac{\sum_i {(y_i - \hat{y}_i)}^2}{\sum_i {y_i}^2}}
    	\end{equation}
    	\rev{where $y_i$ are actual returns and $\hat{y}_i$ are predictions. We use a zero-return baseline (i.e., $\bar{y} = 0$) rather than the historical mean, making the denominator $\sum_i {y_i}^2$. This baseline represents the naive strategy of always predicting zero return. Negative $\rTwo$ values occur when models perform worse than this zero-return baseline, which is common in financial time series due to their inherently low signal-to-noise ratio.}
...
Hyperparameters for all models were chosen based on 2020 validation performance (focusing mainly on the 21-day horizon) and are detailed in Appendix B. \simp{The TFT needed more careful tuning of learning rate and regularization because of its complexity, while simpler models like RNNs were less likely to overfit given our data size. Both RNN and TFT models predict multiple horizons simultaneously from multi-output architectures, while tabular models are trained separately for each horizon; we chose this approach to match how these models are typically used in practice.}

\section{Results}

Table~\ref{tab:main_results} summarizes performance across horizons. \rev{All $\rTwo$ values are negative or near-zero, reflecting market efficiency rather than model failure—when models perform worse than predicting zero return, negative $\rTwo$ is expected in equity prediction due to low signal-to-noise ratios and random-walk characteristics. The relative differences remain meaningful: LSTM's near-zero values suggest better calibration, while Random Forest's highly negative values indicate overfitting.}

\rev{These results highlight why \rmse{} and \da{} are essential complements to $\rTwo$: while $\rTwo$ measures variance explained, \rmse{} quantifies prediction accuracy and \da{} captures directional correctness—more valuable for trading applications.}
...
\begin{table*}[t]
    \centering
    \caption{Table I (Main results). Each cell shows mean $\pm$ std over 3 random seeds unless otherwise noted. \textbf{All \rTwo{} values computed against zero-return baseline} ($b_i = 0$). Metrics computed from our aggregated results file (\texttt{Table1\_EXP-A.csv}). \rev{LSTM is best at $h{=}21$ (lowest RMSE 0.05084; highest DA 55.0\%), while Ridge wins $h{=}5$ (RMSE 0.03651), and RF tops DA at $h{=}1$ (51.9\%). TFT is competitive but does not dominate on this daily panel.}}
...
\rev{LSTM achieves both lowest \rmse{} and highest \da{} at $h=21$, demonstrating economically valuable signals despite minimal variance explanation. This highlights why \rmse{} and \da{} provide crucial complementary information for trading applications beyond $\rTwo$.}
