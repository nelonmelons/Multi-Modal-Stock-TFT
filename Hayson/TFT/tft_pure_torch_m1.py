...
class SimpleTFT(nn.Module):
    """Simplified TFT model implementation for M1 compatibility."""
    
    def __init__(self, input_size, hidden_size=64, num_heads=4, dropout=0.1, seq_len=30, prediction_len=1):
        super().__init__()
        ...
        # Multi-head attention (simplified transformer)
    ...
    def forward(self, x):
        ...
        output = self.norm2(attended + ffn_output)
                ...
                if isinstance(batch, tuple) and len(batch) >= 2:
                    ...
                    if isinstance(x, dict):
                        ...
                        if encoder_features and decoder_features:
                            ...
                            for key, value in x.items():
                                if torch.is_tensor(value) and 'target' not in key.lower() and 'scale' not in key.lower():
                    ...
                    if torch.is_tensor(target):
                        ...
                        if features.shape[0] == target.shape[0]:
                            ...
                            if i == 0:
                                print(f"   DEBUG: Successfully processed batch - X shape: {features.shape}, Y shape: {target.shape}")
                        else:
                            if i == 0:
        ...
        for batch_idx, batch_item in enumerate(train_pbar):
            try:
                ...
                if epoch == 0 and batch_idx == 0:
                    ...
                    print(f"   DEBUG: Batch item content: {batch_item if not torch.is_tensor(batch_item) else 'tensor'}")
                    if isinstance(batch_item, (tuple, list)):
                        print(f"   DEBUG: Batch item length: {len(batch_item)}")
                        for j, item in enumerate(batch_item):
                            print(f"   DEBUG: Item {j} type: {type(item)}, shape: {item.shape if torch.is_tensor(item) else 'N/A'}")
                ...
                if isinstance(batch_item, (tuple, list)) and len(batch_item) == 2:
                    ...
                    print(f"   ERROR: Unexpected batch format: {type(batch_item)}, length: {len(batch_item) if hasattr(batch_item, '__len__') else 'N/A'}")
                    continue
