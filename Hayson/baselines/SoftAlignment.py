...
class SoftAlignModelLoop(nn.Module):
    """
    Memory-efficient soft attention model computing attention per time step.
    """
    def __init__(self, embed_dim=128, hidden_dim=128, input_dim=128):
        super().__init__()
        self.encoder_rnn = nn.GRU(input_dim, hidden_dim, batch_first=True)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        s_t = torch.zeros(1, B, encoder_outputs.size(2), device=x.device)  # (1, B, H)
        # Last input as previous output
        y_prev = x[:, -1:, :]  # (B, 1, D)
        # Compute energies individually to save memory
        energies = []
        s = s_t.squeeze(0)  # (B, H)
        for t in range(T):
            h_t = encoder_outputs[:, t, :]  # (B, H)
            energy_t = self.v(torch.tanh(self.W_h(h_t) + self.W_s(s)))  # (B, 1)
            energies.append(energy_t)
        # Stack energies and compute attention
        e = torch.stack(energies, dim=1)  # (B, T, 1)
        alpha = torch.softmax(e, dim=1)  # (B, T, 1)
        # Context vector
        context = torch.sum(alpha * encoder_outputs, dim=1, keepdim=True)  # (B, 1, H)
        # Decode one step
        decoder_input = torch.cat([y_prev, context], dim=-1)  # (B, 1, D+H)
        output, _ = self.decoder_rnn(decoder_input, s_t)
