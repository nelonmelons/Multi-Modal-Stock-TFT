...
def build_features(stock_df: pd.DataFrame,
    ...
    df = add_target_variable_temporal_safe(df, predict_len, split_date, is_training)
...
def final_cleanup_temporal_safe(df: pd.DataFrame, split_date: str = None, 
    ...
    # Ensure all required columns exist
    required_cols = ['symbol', 'date', 'time_idx']
    # Check for at least one target column
    if not any(col.startswith('target_') for col in df_clean.columns) and 'target' not in df_clean.columns:
        print("Warning: No 'target' or 'target_*' columns found.")
