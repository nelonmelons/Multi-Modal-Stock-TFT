...
def get_data_loader_with_module(symbols: List[str], start: str, end: str,
        ...
        reference_datamodule: Optional[Any] = None) -> "NumericDataModule":
    """
    ...
    Returns:
        A NumericDataModule with .train_loader and .val_loader for debugging and analysis
    """
    print(f"Starting TFT data pipeline for symbols: {symbols}")
    print(f"Date range: {start} to {end}")
    print(f"Encoder length: {encoder_len}, Predict length: {predict_len}")
    ...
    print("7. Creating numeric DataModule and DataLoaders...")
    # Use simple numeric DataModule (no reference or forecasting)
    datamodule = NumericDataModule(
        feature_df=feature_df,
        split_date=split_date or end,
    ...
    # Returning DataModule object only for attribute access; dataloader accessible via datamodule.train_loader / val_loader
    return datamodule
