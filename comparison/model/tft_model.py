...
class TFT(nn.Module):
    ...
    def __init__(
        ...
        self.sector_embedder = None
        if sector_embedder_params:
            self.sector_embedder = SectorEmbedder(**sector_embedder_params)
            fusion_input_dim = hidden_size + news_downsample_dim + sector_embedder_params['output_dim']
        else:
            fusion_input_dim = hidden_size + news_downsample_dim
    ...
    def forward(self, x, news=None, sector_ids=None, subsector_ids=None):
        """
        ...
        # Handle sector embedding (optional)
        sector_embedding = None
        if self.sector_embedder and sector_ids is not None and subsector_ids is not None:
            sector_embedding = self.sector_embedder(sector_ids, subsector_ids)
        ...
        if news is not None:
            ...
            # Concatenate
            fusion_inputs = [last_hidden, news_down]
        else:
            ...
            fusion_inputs = [last_hidden, news_padding]

        if sector_embedding is not None:
            fusion_inputs.append(sector_embedding)

        fusion = torch.cat(fusion_inputs, dim=-1)
